...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    try {
      console.log('📝 Step 2: 검증된 콘텐츠 20개 발행...');
      const publishResponse = await fetch(`${supabaseUrl}/functions/v1/publish-longtail-content?limit=20`, {
        headers: { 
          Authorization: `Bearer ${serviceKey}`,
          'Content-Type': 'application/json',
        },
      ...
      console.log(`✅ ${publishData.published || 0}개 콘텐츠 발행 완료`);
      
      const anomalyDetected = await detectAnomaly(supabase, publishData.published || 0, 20);
      
      if (publishData.published === 0) {
        await sendAlert(
          supabase,
          '🚨 [bustime.site] 콘텐츠 발행 0건',
